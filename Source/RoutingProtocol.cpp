        ...
        for(std::vector<IPv6Address>::iterator it = linkNeighbrs->neighborIfAddr.begin();
            ...
            if(*it == mPersonalAddress) {
                if(linkType == L_LOST_LINK) {
                    ...
                    // Do don't care at this point whether the links are bi-directional or not
                    //
                } else {
...
void RoutingProtocol::handleHelloMessage(shared_ptr<OLSRMessage> message) {
	...
	const vector<LinkTuple> &links = mState.getLinks();
	for (vector<LinkTuple>::const_iterator link = links.begin(); link != links.end(); link++)
	{}

	// We may dont need it. Delete it later or move it to generateHelloMessage or readHelloMessage
	const vector<NeighborTuple> &neighbors = mState.getNeighbors();
	for (vector<NeighborTuple>::const_iterator neighbor = neighbors.begin(); neighbor != neighbors.end(); neighbor++)
	{}
	...
	// We may dont need it. Delete it later or move it to generateHelloMessage or readHelloMessage
	const vector<TwoHopNeighborTuple> &twoHopNeighbors = mState.getTwoHopNeighbors();
	for (vector<TwoHopNeighborTuple>::const_iterator tuple = twoHopNeighbors.begin(); tuple != twoHopNeighbors.end(); tuple++)
	{}
...
void RoutingProtocol::populateTwoHopNeighborSet (const shared_ptr<OLSRMessage> &message, const HelloMessage &hello)
{
	const vector<LinkTuple> &links = mState.getLinks();
	for (vector<LinkTuple>::const_iterator link = links.begin(); link != links.end(); link++)
	{
		...
		vector<HelloMessage::LinkMessage> linkMessages = hello.getLinkMessages();
		for (vector<HelloMessage::LinkMessage>::const_iterator linkMessage = linkMessages.begin ();
			linkMessage != linkMessages.end (); linkMessage++)
		{
			int neighborType= 0; // = (linkMessage->linkCode >> 2) & 0x3;
			for (vector<IPv6Address>::const_iterator nb2hopAddr_iter = linkMessage->neighborInterfaceAddresses.begin ();
				nb2hopAddr_iter != linkMessage->neighborInterfaceAddresses.end (); nb2hopAddr_iter++)
			{
...
void RoutingProtocol::handleTCMessage(shared_ptr<OLSRMessage> message) {
	...
	const vector<IPv6Address> &neighborAddresses = tc.getNeighborAddresses();
	for(vector<IPv6Address>::const_iterator i = neighborAddresses.begin(); i != neighborAddresses.end(); i++)
	{
	...
	// We may dont need it. Delete it later or move it to generateHelloMessage or readHelloMessage
	const vector<TopologyTuple> & topology = mState.getTopologySet();
	for(vector<TopologyTuple>::const_iterator tuple = topology.begin(); tuple != topology.end(); tuple++)
	{}
}
...
// 	for (vector<NeighborTuple>::const_iterator neighbor = mState.GetNeighbors().begin();
// 	     neighbor != mState.GetNeighbors().end(); neighbor++)
// 	{
// 		if (neighbor->status == NeighborTuple::STATUS_SYM) // I think that we need this check
// 		{
// 			N.push_back (*neighbor);
...
// 	vector<TwoHopNeighborTuple> N2;
// 	for (vector<TwoHopNeighborTuple>::const_iterator twoHopNeigh = mState.GetTwoHopNeighbors().begin();
// 	     twoHopNeigh != mState.GetTwoHopNeighbors().end(); twoHopNeigh++)
// 	{
// 		if (twoHopNeigh->twoHopNeighborAddr == m_mainAddress) {
// 			continue;
// 		}
...
// 	// 3. Add to the MPR set those nodes in N, which are the *only*
// 	// nodes to provide reachability to a node in N2.
// 	set<Ipv6Address> coveredTwoHopNeighbors;
// 	for (vector<TwoHopNeighborTuple>::iterator twoHopNeigh = N2.begin(); twoHopNeigh != N2.end(); twoHopNeigh++)
// 	{
// 		vector<NeighborTuple>::const_iterator onlyNeighbor = N.end();
...
// 		// least one node in the MPR set, and which are reachable
// 		// through this 1-hop neighbor
// 		map<int, vector<const NeighborTuple *> > reachability;
// 		set<int> rs;
// 		for (vector<NeighborTuple>::iterator it = N.begin(); it != N.end(); it++)
// 		{
// 			NeighborTuple const &nb_tuple = *it;
...
// 				continue;
// 			}
// 			for (vector<const NeighborTuple *>::iterator it2 = reachability[r].begin();
// 			     it2 != reachability[r].end(); it2++)
// 			{
// 				const NeighborTuple *nb_tuple = *it2;
// 				if (max == NULL || nb_tuple->willingness > max->willingness)
...
=======
// 	const InterfaceAssociationTuple *tuple = mState.FindIfaceAssocTuple (iface_addr);
>>>>>>> RoutingProtocol Update
...
// 	// 2. The new routing entries are added starting with the symmetric neighbors (h=1) as the destination nodes.
// 	const vector<NeighborTuple> &neighborSet = mState.GetNeighbors();
// 	for (vector<NeighborTuple>::const_iterator it = neighborSet.begin(); it != neighborSet.end(); it++)
// 	{
// 		NeighborTuple const &nb_tuple = *it;
// 		if (nb_tuple.status == NeighborTuple::STATUS_SYM)
...
// 			bool nb_main_addr = false;
// 			const LinkTuple *lt = NULL;
// 			const vector<LinkTuple> &vector<LinkTuple> = mState.GetLinks();
// 			for (vector<LinkTuple>::const_iterator it2 = vector<LinkTuple>.begin(); it2 != vector<LinkTuple>.end(); it2++)
// 			{
// 				LinkTuple const &linkTuple = *it2;
// 				if ((setPersonalAddress (linkTuple.neighborIfaceAddr) == nb_tuple.neighborMainAddr))
// 				{
// 					lt = &linkTuple;
// 					AddEntry (linkTuple.neighborIfaceAddr, linkTuple.neighborIfaceAddr, linkTuple.localIfaceAddr, 1);
// 					if (linkTuple.neighborIfaceAddr == nb_tuple.neighborMainAddr)
// 					{
// 						nb_main_addr = true;
// 					}
// 				}
...
// 		TwoHopNeighborTuple const &nb2hop_tuple = *it;
// 		// a 2-hop neighbor which is not a neighbor node or the node itself
// 		if (mState.FindNeighborTuple(nb2hop_tuple.twoHopNeighborAddr)) { continue; }
// 		if (nb2hop_tuple.twoHopNeighborAddr == m_mainAddress) { continue; }
...
// 	// such that:								R_dest_addr	== I_main_addr	(of the multiple interface association entry)
// 	// AND there is no routing entry such that:	R_dest_addr	== I_iface_addr
// 	const vector<InterfaceAssociationTuple> &ifaceAssocSet = mState.GetIfaceAssocSet();
// 	for (vector<InterfaceAssociationTuple>::const_iterator it = ifaceAssocSet.begin(); it != ifaceAssocSet.end(); it++)
// 	{
// 		InterfaceAssociationTuple const &tuple = *it;
// 		RoutingTableEntry entry1, entry2;
