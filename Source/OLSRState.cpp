...
bool OLSRState::findMprAddress (const IPv6Address  &address) {
    set<IPv6Address>::iterator it = mMprSet.find(address);
    return (it != mMprSet.end());

}

void OLSRState::setMprSet (set<IPv6Address> mprSet) {
    mMprSet = mprSet;
}
...
MprSelectorTuple* OLSRState::findMprSelectorTuple (IPv6Address &mainAddr) {
    for (vector<MprSelectorTuple>::iterator it = mMprSelectorSet.begin();
...
string OLSRState::printMprSelectorSet() const
{
    stringstream os;
    os << "[";
    for (vector<MprSelectorTuple>::const_iterator iter = mMprSelectorSet.begin();
            iter != mMprSelectorSet.end(); iter++) {
        vector<MprSelectorTuple>::const_iterator next = iter;
        next++;
        os << iter->mainAddr;
...
{
    for (vector<NeighborTuple>::iterator it = mNeighborSet.begin(); it != mNeighborSet.end(); it++) {
...
{
    for (vector<NeighborTuple>::const_iterator it = mNeighborSet.begin(); it != mNeighborSet.end(); it++) {
...
{
    for (vector<TwoHopNeighborTuple>::iterator it = mTwoHopNeighborSet.begin(); it != mTwoHopNeighborSet.end(); it++) {
...
{
    for (vector<TwoHopNeighborTuple>::iterator it = mTwoHopNeighborSet.begin(); it != mTwoHopNeighborSet.end();) {
...
{
    for (vector<LinkTuple> ::iterator it = mLinkSet.begin(); it != mLinkSet.end(); it++) {
...
{
    for (vector<TopologyTuple>::iterator it = mTopologySet.begin(); it != mTopologySet.end(); it++) {
...
{
    for (vector<TopologyTuple>::iterator it = mTopologySet.begin(); it != mTopologySet.end();) {
        if (it->lastAddr == lastAddr && it->sequenceNumber < ansn) {
...
{
    for (vector<InterfaceAssociationTuple>::iterator it = mInterfaceAssociationSet.begin(); it != mInterfaceAssociationSet.end(); it++) {
...
{
    for (vector<InterfaceAssociationTuple>::const_iterator it = mInterfaceAssociationSet.begin(); it != mInterfaceAssociationSet.end(); it++) {
        if (it->ifaceAddr == ifaceAddr)
            return &(*it);
    }
    return NULL;
...
vector<IPv6Address> OLSRState::findNeighborInterfaces (const IPv6Address &neighborMainAddr) const
{
